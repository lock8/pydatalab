...
class Csv(object):
  ...
  def _create_external_datasource(self, skip_header_rows):
  ...
  def _get_gcs_csv_row_count(self, external_datasource):
    import google.datalab.bigquery as bq
    results = bq.Query('SELECT count(*) from data',
                       data_sources={'data': external_datasource}).execute().results
  ...
  def sample_to(self, count, skip_header_rows, strategy, target):
    ...
    if strategy == 'BIGQUERY':
      ...
      external_datasource = self._create_external_datasource(skip_header_rows)
      row_count = self._get_gcs_csv_row_count(external_datasource)
      query = bq.Query('SELECT * from data', data_sources={'data': external_datasource})
